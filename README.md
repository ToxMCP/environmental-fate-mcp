...
- Run-level scientific review packets and briefs now also expose post-release directionality lines so reviewers can see whether retained release-stop mass remains above, at, or below the one-turnover anchor as the recovery window progresses
- Run-level scientific review packets and briefs now also expose post-release pace lines so reviewers can see whether a post-release window is still before, at, or beyond the combined-loss half-recovery pace rather than inferring recovery pace from the one-turnover boundary alone
- Run-level scientific review packets and briefs now also expose post-release pace directionality lines so reviewers can see whether retained release-stop mass remains above, at, or materially below the 50% combined-loss recovery anchor as the recovery window extends
...
- The experimental advective family now also carries a governed post-release directionality claim, so sub-boundary, boundary, and beyond-boundary retained-mass progression is claim-covered rather than inferred from isolated recovery examples
...
- Scientific methods dossiers now also expose highlighted post-release half-recovery pace claims in the advective family, so combined-loss recovery pace remains visible as a distinct native-science authority layer rather than being folded into one-turnover flushing language
- Scientific methods dossiers now also expose highlighted post-release half-recovery directionality claims in the advective family, so reviewer-facing recovery authority covers not just crossing the 50% anchor but continuing materially below it under the same governed loss constants
