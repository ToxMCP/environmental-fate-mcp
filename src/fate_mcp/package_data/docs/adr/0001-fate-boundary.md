# ADR 0001: Environmental Fate MCP Boundary
...
Environmental Fate MCP owns source-to-concentration environmental workflows:
...
Environmental Fate MCP explicitly excludes:
...
The ToxMCP suite should split by pathway semantics and input grammar, not by route alone. Environmental Fate MCP accepts environmental release assumptions and returns auditable concentration outputs that downstream systems can consume without knowing the underlying fate engine.
Traditional Chinese Medicine, herbal medicine, and supplement labels do not change that split:
Environmental Fate MCP still stops at concentration outputs, and downstream routing depends on whether the
