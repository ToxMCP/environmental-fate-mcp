...
| `R7` | The `external_adapter_canonical_equivalence_v1` claim remains the only `worksheetStatus=missing` entry; no model-family worksheet pack exists for the adapter family | `confirmed` | Add a hand-worked `external_adapter_canonical_surface_signature_fixture` to `BENCHMARK_FIXTURES` pinning the canonical surface signature (single surface_water concentration of 1.25e-02 mg/L) the JSON adapter fixture must reproduce; generalize the worksheet packager once more with `ADAPTER_WORKSHEET_PACK_DIR`; wire `_build_adapter_worksheet_manifest_report` into the report registry; ship the pack alongside the manifest at `docs/releases/v0.5.0/adapter-worksheet-pack/`. Evidence family stays `public_method_description_plus_internal_oracle`: the adapter family is a normalization-parity lane, not a kernel-physics lane, so the pack adds reviewability without false scientific-equivalence promotion | `release-artifacts` | `1` | `src/fate_mcp/benchmarks.py::external_adapter_canonical_surface_signature_fixture`, `src/fate_mcp/release_artifacts.py::_build_adapter_worksheet_manifest_report`, `tests/test_adapter_worksheet_pack.py` (5 tests), `docs/releases/v0.5.0/adapter-worksheet-pack/` (2 files), `docs/releases/v0.5.0/adapter-worksheet-manifest.json` | `implemented` |
...
| `R12` | No multi-stage governed workflow (queue → review board → owner handoff → remediation → signoff) analogous to Dietary's owner pipeline; review surfaces stop at the `preview` stage | `confirmed` | Ship a linear five-stage state machine (`QUEUED` → `UNDER_REVIEW_BOARD` → `OWNER_HANDOFF` → `OWNER_REMEDIATION` → `OWNER_SIGNOFF`) with `enqueue_scientific_follow_up` / `advance_scientific_follow_up` helpers, per-transition acceptance-evidence requirement, content-addressed integrity hashing, and fail-closed rejection of non-linear / terminal-stage transitions. v1 is a public integration function (no MCP tool wrap yet); linear-only state machine (branching deferred); caller-supplied stage payloads (no per-stage schemas yet) | `runtime` | `1` | `src/fate_mcp/models.py::ScientificFollowUpBundle`, `src/fate_mcp/integrations/scientific_follow_up.py`, `tests/test_scientific_follow_up.py` (12 tests), `docs/scientific_follow_up.md` | `implemented` |
