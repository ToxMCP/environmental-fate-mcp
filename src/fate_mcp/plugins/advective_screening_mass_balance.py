...
class AdvectiveScreeningMassBalancePlugin(ReferenceMassBalancePlugin):
    ...
    @staticmethod
    def _safe_advective_constant(residence_time_days: float) -> float:
        if residence_time_days <= 0.0:
            raise FateValidationError(
                code="non_positive_residence_time",
                message=(
                    f"Residence time must be positive, got {residence_time_days} days. "
                    "Advective clearance cannot be computed for non-positive residence times."
                ),
                suggestion="Provide a positive advective residence time or switch to a non-advective model family.",
            )
        return 1.0 / residence_time_days
