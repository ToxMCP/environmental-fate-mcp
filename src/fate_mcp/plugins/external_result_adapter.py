...
ADAPTER_IMPORT_PROFILES = [
    ...
    AdapterImportProfile(
        ...
        accepted_extensions=[".csv"],
        accepted_modes=[RunMode.STEADY_STATE, RunMode.TIME_BUCKET],
        description="Concrete legacy desktop screening export normalized into Environmental Fate MCP concentrations.",
    ),
    AdapterImportProfile(
        ...
        accepted_extensions=[".csv"],
        accepted_modes=[RunMode.STEADY_STATE, RunMode.TIME_BUCKET],
        description="EUSES screening export normalized into Environmental Fate MCP concentrations.",
        semantic_mapping=AdapterSemanticMapping(
            semantic_loss=SemanticLossClassification.MINOR,
            compartment_semantics="EUSES regional compartments map to Environmental Fate MCP generic compartments.",
    ...
    for row_number, row in enumerate(reader, start=2):
        ...
        if compartment_info is None:
            raise FateValidationError(
                ...
                suggestion="Map the compartment label into a supported Environmental Fate MCP compartment before importing.",
        ...
        normalized_surfaces.append(
            ConcentrationSurface(
                ...
                limitations=[
                    ...
                    LimitationNote(
                        code="adapter_trace_disclaimer",
                        message=(
                            "Native calculation trace from the source engine is unavailable. "
                            "The audit trail resumes at the normalization boundary."
                        ),
                    ),
                    *unsupported_limitations,
                    *(
...
class ExternalResultAdapterHarnessPlugin(ReferenceMassBalancePlugin):
    ...
    limitations = [
        "External-result adapter harness normalizes an engine-like payload shape into Environmental Fate MCP outputs.",
