...
def test_error_quality_flag_escalates_scientific_review_outcome() -> None:
    runtime = FateRuntime(Path(__file__).resolve().parents[1])
    scenario = runtime.build_environmental_release_scenario(
        BuildEnvironmentalReleaseScenarioRequest(
    ...
    # Inject an ERROR quality flag into the first surface
    result.surfaces[0].quality_flags.append(
        QualityFlag(
            code="fake_critical_error",
            severity=Severity.ERROR,
            message="A synthetic ERROR flag for testing escalation.",
        )
    )
    preview = preview_scientific_review_outcome(
        PreviewScientificReviewOutcomeRequest(scenario=scenario, result=result),
    ...
    assert preview.review_outcome == "escalate_model_review"
    assert any("ERROR-quality flags are present" in line for line in preview.governing_rule_lines)
